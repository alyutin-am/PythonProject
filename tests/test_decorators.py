...
def test_log_decorator_without_file(capsys):
...
def test_log_decorator_with_file(tmp_path):
...
def test_log_decorator_timestamps():
...
def test_log_decorator_with_kwargs(tmp_path):
    """Проверяем обработку kwargs."""
    test_file = tmp_path / "test_log.txt"

    @log(str(test_file))
    def kwargs_func(a, b=0):
        return a + b
